"""
Bybit Futures (Linear/USD-M) WebSocket Collector
Identical to bybit_spot.py except WS URL.
"""
import asyncio
import os
import random
...
SYMBOLS_FILE = os.path.join(os.path.dirname(__file__), "..", "dictionaries", "subscribe", "bybit", "bybit_futures.txt")
...
def parse_message(data: dict, log) -> dict | None:
    """Parse Bybit V5 tickers message (linear futures — identical format to spot)."""
    op = data.get("op")
