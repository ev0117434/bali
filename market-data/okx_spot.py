...
SYMBOLS_FILE = os.path.join(
    os.path.dirname(__file__), "..", "dictionaries", "subscribe", "okx", "okx_spot.txt"
)
...
def _to_native(symbol: str) -> str:
    """BTCUSDT → BTC-USDT (OKX spot instId format)."""
...
def parse_message(data: dict, log) -> dict | None:
    """
    Parse OKX V5 tickers channel message.
...
async def ws_worker(symbols: list[str], conn_id: int, redis_client, shutdown_event: asyncio.Event, log):
    ...
    attempt = 0
    # Convert normalized symbols to OKX native instId format
