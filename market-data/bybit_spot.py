...
SYMBOLS_FILE = os.path.join(os.path.dirname(__file__), "..", "dictionaries", "subscribe", "bybit", "bybit_spot.txt")
...
def parse_message(data: dict, log) -> dict | None:
    ...
    Returns None for control messages (ping/pong/subscribe).
    """
    op = data.get("op")
    ...
    if op == "subscribe":
        if data.get("success"):
            log.info("ws_subscribed", args=data.get("args", []))
        return None

    # Bybit ping request → caller should send pong (handled in read_loop)
    if op == "ping":
        return None

    # Bybit pong response (heartbeat from server) → ignore
    if op == "pong":
        return None

    # Data message: topic starts with "tickers."
...
async def ping_loop(ws, shutdown_event: asyncio.Event, interval: int, log):
    """Send periodic ping to Bybit to keep connection alive."""
