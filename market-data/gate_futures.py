"""
Gate.io Futures (USDT Perpetual) WebSocket Collector
Subscribes to futures.book_ticker channel, writes best bid/ask to Redis.

Gate.io V4 protocol specifics:
- Subscribe: {"time": ts, "channel": "futures.book_ticker", "event": "subscribe", "payload": [...]}
  Payload is a list of native symbols (BTC_USDT), sent in batches of BATCH_SIZE
- Ping: {"time": ts, "channel": "futures.ping"}  (JSON, not plain text)
- Response: {"channel": "futures.book_ticker", "event": "update",
             "time": ts_sec, "result": {"s": "BTC_USDT", "b": bid, "B": bid_qty,
                                        "a": ask, "A": ask_qty}}
- Futures URL: wss://fx-ws.gateio.ws/v4/ws/usdt (separate from spot)
"""
import asyncio
import os
import random
...
SYMBOLS_FILE = os.path.join(
    os.path.dirname(__file__), "..", "dictionaries", "subscribe", "gate", "gate_futures.txt"
)
...
def _to_native(symbol: str) -> str:
    """BTCUSDT → BTC_USDT (Gate.io native format, same for futures)."""
...
def parse_message(data: dict, log) -> dict | None:
    """
    Parse Gate.io V4 futures.book_ticker update message.
...
async def ping_loop(ws, shutdown_event: asyncio.Event, interval: int, log):
    """Send periodic JSON ping to Gate.io futures."""
    while not shutdown_event.is_set():
        await asyncio.sleep(interval)
        if shutdown_event.is_set():
            break
        try:
            await ws.send(orjson.dumps({"time": _ts(), "channel": "futures.ping"}).decode())
        except Exception:
            break
