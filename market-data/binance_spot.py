...
Subscribes to bookTicker stream for all symbols, writes best bid/ask to Redis.
"""
import asyncio
import os
import random
...
SYMBOLS_FILE = os.path.join(os.path.dirname(__file__), "..", "dictionaries", "subscribe", "binance", "binance_spot.txt")
REDIS_CHANNEL = f"md:updates:{EXCHANGE}:{MARKET}"

# In-memory buffer for Redis-down scenarios
from collections import deque
_redis_buffer: deque = deque(maxlen=1000)


def parse_message(data: dict, log) -> dict | None:
    """Parse Binance bookTicker message. Returns None if message should be skipped."""
    # Subscribe confirmation: {"result": null, "id": N}
    if "result" in data:
    ...
    return {
        ...
        "ask_qty": data.get("A", ""),
        "last": "",
        "ts_exchange": "0",  # bookTicker spot has no exchange timestamp
    }


async def write_redis(redis_client, parsed: dict, ts_received: int, log) -> bool:
    """Write parsed data to Redis via pipeline. Returns True on success."""
    ...
    try:
        ...
        await pipe.execute()
        # Flush buffer if Redis recovered
        if _redis_buffer:
            flushed = 0
            while _redis_buffer:
...
async def ws_worker(symbols: list[str], conn_id: int, redis_client, shutdown_event: asyncio.Event, log):
    """Single WebSocket connection worker with infinite reconnect."""
