...
Файловая структура:
  signals/{direction}/{symbol}_{ts_signal}.jsonl

Первая строка файла — исходный сигнал (event=signal).
Последующие строки — снапшоты с текущими ценами.
...
# Источники данных по направлению (копия из spread_scanner.py)
_SOURCES = {
...
async def snapshot_task(redis, signal_data: dict, log) -> None:
    ...
    Каждая строка — JSON с текущими ценами из Redis + вычисленный spread_pct.
    """
    ...
    # signals/{direction}/{symbol}_{ts_signal}.jsonl
    out_dir  = os.path.join(SIGNALS_DIR, direction)
    os.makedirs(out_dir, exist_ok=True)
    filepath = os.path.join(out_dir, f"{symbol}_{ts_signal}.jsonl")

    n        = 0
    deadline = time.monotonic() + SNAPSHOT_DURATION_S

    with open(filepath, "a", encoding="utf-8", buffering=1) as f:
        # Первая строка — исходный сигнал
        f.write(orjson.dumps({"event": "signal", **signal_data}).decode() + "\n")
        ...
        while time.monotonic() < deadline:
            ...
            try:
                ...
                (buy_ask, buy_qty, buy_ts), (sell_bid, sell_qty, sell_ts) = await pipe.execute()
    ...
    log.info("snapshot_done", symbol=symbol, direction=direction, rows=n, file=filepath)
    ...
    try:
        async for message in pubsub.listen():
            ...
            task = asyncio.create_task(
                ...
                name=f"snap:{sig_data.get('direction')}:{sig_data.get('symbol')}",
            )
            tasks.add(task)
