...
Если chunk_num изменился для данного (exchange, market, symbol):
  DEL md:hist:{exchange}:{market}:{symbol}:{slot}   ← стираем старый чанк

  Если (slot, chunk_num) впервые встречен:
    HSET md:hist:registry {slot} "{chunk_num}:{start_ts_ms}"  ← реестр
...
`md:hist:registry` — единый Hash, который отвечает на вопрос «какой временно́й диапазон хранится в каждом слоте прямо сейчас». Обновляется не чаще одного раза в 20 минут (ровно при смене чанка). Потребители используют его для верификации: сравнивают `chunk_num` из реестра с математически вычисленным `unix_time // 1200`.

```
HGETALL md:hist:registry
→  slot 0: chunk 1451, начало 1741234800000 мс  (= 2026-03-05 14:00 UTC)
   slot 1: chunk 1452, начало 1741236000000 мс  (= 2026-03-05 14:20 UTC)
   slot 2: chunk 1453, начало 1741237200000 мс  (= 2026-03-05 14:40 UTC)  ← текущий
   slot 3: chunk 1449, начало 1741230000000 мс  (= 2026-03-05 13:00 UTC)
   slot 4: chunk 1450, начало 1741231200000 мс  (= 2026-03-05 13:20 UTC)
```
