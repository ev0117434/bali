...
> **Язык:** Python 3.10+
> **Охват:** все скрипты репозитория — коллекторы, мониторы, сканер, snapshot-логгер, лончер, генератор словарей
...
8. [price_history.py — хранение истории цен](#8-price_historypy--хранение-истории-цен)
9. [run.py — лончер всех процессов](#9-runpy--лончер-всех-процессов)
10. [dictionaries/main.py — генератор словарей](#10-dictionariesmainpy--генератор-словарей)
11. [Redis: ключи и каналы](#11-redis-ключи-и-каналы)
12. [Переменные окружения](#12-переменные-окружения)
13. [Форматы файлов и выходных данных](#13-форматы-файлов-и-выходных-данных)
14. [Обработка ошибок и устойчивость](#14-обработка-ошибок-и-устойчивость)
...
└──────────────┬──────────────────────────────┘
               ...
               ▼
         Redis (db 0)
           md:{exchange}:{market}:{symbol}   ← hash-ключи (текущая цена)
           md:updates:{exchange}:{market}    ← pub/sub-каналы
           md:hist:{exchange}:{market}:{symbol}:{slot}  ← история (list)
               │
       ┌───────┼────────────────┐
       ▼       ▼                ▼
stale_monitor  latency_monitor  price_history
 (SCAN каждые   (SCAN каждые    (psubscribe md:updates:*,
  30 с)          10 с)           пишет чанки по 20 мин)
               │
               ▼
         spread_scanner
          (каждые 200 мс)
...
## 8. price_history.py — хранение истории цен

**Файл:** `market-data/price_history.py`

### Концепция

Сервис постоянно держит в Redis скользящую историю цен глубиной **100 минут** (5 чанков × 20 минут) по каждому источнику и каждому торговому символу. Работает как отдельный процесс, не требует изменений в коллекторах.

### Логика чанков (скользящее окно)

```
chunk_num = unix_time // CHUNK_SECONDS   # глобальный индекс (растёт непрерывно)
slot      = chunk_num % MAX_CHUNKS       # номер слота из 5 (0..4)
```

Поведение при переходе к новому чанку:

| chunk_num | slot | Действие |
|-----------|------|----------|
| 1 | 1 | DEL слот 1 (пустой), начать писать чанк 1 |
| 2 | 2 | DEL слот 2, начать писать чанк 2 |
| 3 | 3 | DEL слот 3, начать писать чанк 3 |
| 4 | 4 | DEL слот 4, начать писать чанк 4 |
| 5 | 0 | DEL слот 0, начать писать чанк 5 |
| **6** | **1** | **DEL слот 1 (стирает чанк 1)**, начать писать чанк 6 |
| **7** | **2** | **DEL слот 2 (стирает чанк 2)**, начать писать чанк 7 |

Старый чанк удаляется автоматически: слот `chunk_num % 5` всегда занимает ровно один чанк — при приходе нового данные предыдущего цикла затираются.

### Redis-схема

**Ключи истории (List):**
```
Ключ:    md:hist:{exchange}:{market}:{symbol}:{slot}
Тип:     List
Элемент: "{ts_redis_ms}:{bid}:{ask}"
```

**Примеры ключей:**
```
md:hist:binance:spot:BTCUSDT:0
md:hist:binance:futures:BTCUSDT:3
md:hist:bybit:spot:ETHUSDT:2
md:hist:gate:futures:SOLUSDT:4
```

**Пример содержимого:**
```
LRANGE md:hist:binance:spot:BTCUSDT:2 0 3
→ "1710754800000:50000.10:50001.20"
   "1710754800105:50000.15:50001.25"
   "1710754800210:49999.90:50001.00"
   ...
```

На каждый источник (exchange + market) и каждый символ создаётся **5 ключей** (слоты 0–4). В любой момент активен один слот — остальные содержат предыдущие чанки.

**Реестр слотов (Hash):**
```
Ключ:   md:hist:registry
Тип:    Hash
Поле:   "{slot}"            (строка "0".."4")
Значение: "{chunk_num}:{start_ts_ms}"
```

Обновляется один раз на каждую ротацию слота (раз в 20 минут). Позволяет потребителям узнать временно́й диапазон каждого слота и проверить, не устарели ли данные.

**Пример содержимого:**
```
HGETALL md:hist:registry
→ "0" "1451:1741234800000"   ← слот 0 = чанк 1451, начался 1741234800000 мс
   "1" "1452:1741236000000"   ← слот 1 = чанк 1452, ...
   "2" "1453:1741237200000"
   "3" "1449:1741230000000"
   "4" "1450:1741231200000"
```

Временно́й диапазон чанка: `[start_ts_ms, start_ts_ms + CHUNK_SECONDS * 1000)`

### Архитектура

```
Redis pub/sub (psubscribe md:updates:*)
       │
       │  pmessage: channel="md:updates:binance:spot"
       │            data={"symbol":"BTCUSDT","key":"md:binance:spot:BTCUSDT"}
       ▼
  Буферизация (до BATCH_SIZE=200 или BATCH_TIMEOUT_MS=100 мс)
       │
       │  pipeline HMGET  bid, ask, ts_redis
       ▼         (один round-trip для всего батча)
  RotationTracker: для каждого (exchange, market, symbol)
    ├── chunk_num изменился? → DEL hist_key (стираем старый слот)
    │   └── первый раз для (slot, chunk_num)?
    │       → HSET md:hist:registry {slot} "{chunk_num}:{start_ts_ms}"
    └── RPUSH hist_key "{ts}:{bid}:{ask}"
       │
       │  pipeline (один round-trip)
       ▼
     Redis  md:hist:*:*:*:{slot}
            md:hist:registry
```
...
|---|---|---|
| `HIST_CHUNK_SECONDS` | `1200` | Длина одного чанка (20 минут) |
| `HIST_MAX_CHUNKS` | `5` | Число чанков в буфере (итого 100 минут) |
| `HIST_BATCH_SIZE` | `200` | Максимум обновлений в одном батче |
| `HIST_BATCH_TIMEOUT_MS` | `100` | Максимальная задержка флаша батча (мс) |

### Чтение истории

```bash
# Текущий слот (математически)
SLOT=$(($(date +%s) / 1200 % 5))

# Все записи активного чанка
redis-cli LRANGE "md:hist:binance:spot:BTCUSDT:$SLOT" 0 -1

# Количество точек
redis-cli LLEN "md:hist:binance:spot:BTCUSDT:$SLOT"

# Реестр: временно́й диапазон всех слотов
...
key  = f"md:hist:binance:spot:BTCUSDT:{slot}"

for entry in r.lrange(key, 0, -1):
    ts, bid, ask = entry.split(":")
    print(ts, bid, ask)
```

### Верификация через реестр

Перед чтением данных потребитель может проверить, что слот содержит именно тот чанк, который ожидается:
...
expected_chunk = now_sec // CHUNK_SECONDS
expected_slot  = expected_chunk % MAX_CHUNKS

registry_val   = r.hget("md:hist:registry", str(expected_slot))
# registry_val = "1453:1741237200000"

if registry_val:
    actual_chunk, start_ts_ms = registry_val.split(":")
    if int(actual_chunk) == expected_chunk:
        pass  # данные актуальные — можно читать
    else:
        pass  # price_history не писал в этот слот — данные от старого цикла
```

**Нюанс — реестр не заполнен при первом старте:**
При первом запуске `price_history` реестр пуст (`HGET` вернёт `None`). Он заполняется постепенно: один `HSET` на каждый первый батч символов для данного слота. Обычно все 5 слотов заполнены в течение первого цикла из 5 чанков (100 минут после старта).

### Нюансы

**Нюанс — нет TTL на hist-ключах:**
Ключи `md:hist:*` не имеют TTL — они живут вечно и ротируются через DEL при смене чанка. Это гарантирует доступность истории даже при паузе в обновлениях (например, ночью).

**Нюанс — при старте всегда DEL текущего слота:**
При первом обновлении символа `RotationTracker` считает chunk_num "новым" (ранее не видел) и делает DEL. Это корректно: при перезапуске сервиса старые данные текущего слота будут стёрты и записаны заново с актуальной метки.

**Нюанс — gap в данных:**
Если `price_history` был остановлен, в ключах останутся данные последнего активного чанка. При повторном старте запись продолжится с момента перезапуска — "дыра" останется в том же чанке.

**Нюанс — батчевое чтение:**
После получения pub/sub сообщения сервис не читает bid/ask из сообщения напрямую (там только symbol + key), а делает `HMGET` из основного hash-ключа. Это позволяет всегда читать последние актуальные данные, даже если очередь немного отстала.

### Обязательные события лога

| Событие | Уровень | Когда |
|---|---|---|
| `price_history_started` | INFO | Старт с параметрами chunk_seconds, max_chunks |
| `subscribed` | INFO | psubscribe успешен |
| `chunk_rotated` | DEBUG | Новый чанк для source:symbol, старый слот удалён |
| `registry_updated` | INFO | Реестр обновлён: slot, chunk_num, start_ts_ms |
| `batch_error` | ERROR | Ошибка при обработке батча (Redis недоступен и т.д.) |
| `price_history_stopped` | INFO | Завершение сервиса |

---

## 9. run.py — лончер всех процессов

**Файл:** `run.py` (корень репозитория)

### Что делает

Запускает до 13 дочерних процессов через `subprocess.Popen`, перенаправляет их stdout/stderr в общий лог и в отдельные файлы с ротацией.
...
## 10. dictionaries/main.py — генератор словарей
...
Записываются `price_history.py`. Тип — Redis List. Каждый элемент: `"{ts_ms}:{bid}:{ask}"`.

| Параметр | Значение |
|---|---|
| `slot` | `chunk_num % 5`, где `chunk_num = unix_time // 1200` |
| Длина чанка | 1200 секунд (20 минут) |
| Активных слотов | 5 (100 минут истории) |
| TTL | нет — ключи живут вечно, ротируются через DEL |

**Чтение текущего чанка:**
```bash
redis-cli LRANGE md:hist:binance:spot:BTCUSDT:$(($(date +%s) / 1200 % 5)) 0 -1
```
...
Записывается `price_history.py`. Тип — Redis Hash. Обновляется **один раз при каждой ротации слота** (раз в 20 минут), не при каждом батче.

| Поле | Значение | Описание |
|---|---|---|
| `"0"` .. `"4"` | `"{chunk_num}:{start_ts_ms}"` | Индекс чанка и время начала (мс) |

Потребители читают этот хеш чтобы:
1. Узнать точный временно́й диапазон каждого слота
2. Убедиться, что данные в слоте от ожидаемого цикла (не устаревшие)
...
| Переменная | По умолчанию | Описание |
|---|---|---|
| `HIST_CHUNK_SECONDS` | `1200` | Длина чанка в секундах (20 минут) |
| `HIST_MAX_CHUNKS` | `5` | Число хранимых чанков (100 минут истории) |
...
## 13. Форматы файлов и выходных данных
...
| `price_history_started` | INFO | price_history запущен (chunk_seconds, max_chunks) |
| `subscribed` | INFO | psubscribe на `md:updates:*` успешен |
| `chunk_rotated` | DEBUG | Начат новый чанк для source:symbol, старый слот удалён |
| `registry_updated` | INFO | Реестр обновлён: slot, chunk_num, start_ts_ms |
| `batch_error` | ERROR | Ошибка при обработке батча в price_history |
| `price_history_stopped` | INFO | price_history завершён |
