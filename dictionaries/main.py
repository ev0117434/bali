...
def _make_subscribe_files() -> tuple:
    ...
    Файлы с 'binance_spot' в имени → subscribe/binance/binance_spot.txt (без дублей)
    Файлы с 'bybit_spot'   в имени → subscribe/bybit/bybit_spot.txt (без дублей)
    Возвращает (binance_spot_pairs, bybit_spot_pairs).
    """
    binance_spot_pairs: set = set()
    bybit_spot_pairs: set = set()
    ...
    for comb_file in COMBINATION_DIR.glob("*.txt"):
        ...
        if "binance_spot" in name:
            binance_spot_pairs.update(lines)
        if "bybit_spot" in name:
            bybit_spot_pairs.update(lines)

    bs = sorted(binance_spot_pairs)
    bys = sorted(bybit_spot_pairs)
...
def main() -> None:
    ...
    sub_bn_spot, sub_bb_spot = _make_subscribe_files()
    r['sub_bn_spot'] = len(sub_bn_spot)
    r['sub_bb_spot'] = len(sub_bb_spot)
    print(f"      binance_spot.txt: {len(sub_bn_spot)}, bybit_spot.txt: {len(sub_bb_spot)}")
