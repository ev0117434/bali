...
Порядок выполнения:
  ...
  7. Gate.io REST API — получение всех пар (spot + futures)
  8. Gate.io WebSocket — валидация активности пар (60 сек)
  9. Создание пересечений → combination/
  10. Создание файлов подписки → subscribe/
  11. Итоговый отчёт
...
Файлы подписки (subscribe/):
  ...
  subscribe-файл. Например, binance_spot_gate_futures.txt →
    subscribe/binance/binance_spot.txt  +  subscribe/gate/gate_futures.txt
"""
...
def main() -> None:
    ...
    from gate.gate_pairs       import fetch_pairs as gate_fetch, load_native as gate_load_native
    from gate.gate_ws          import validate_pairs as gate_ws

    r = {}
    t0 = time.time()

    # ── 1. Binance REST ───────────────────────────────────────────────────────
    print("[1/10] Binance REST API: получение пар...", flush=True)
    ...
    # ── 2. Binance WS ─────────────────────────────────────────────────────────
    print("[2/10] Binance WebSocket: валидация (60 сек)...", flush=True)
    t1 = time.time()
    ...
    # ── 3. Bybit REST ─────────────────────────────────────────────────────────
    print("[3/10] Bybit REST API: получение пар...", flush=True)
    ...
    # ── 4. Bybit WS ───────────────────────────────────────────────────────────
    print("[4/10] Bybit WebSocket: валидация (60 сек)...", flush=True)
    t2 = time.time()
    ...
    # ── 5. OKX REST ───────────────────────────────────────────────────────────
    print("[5/10] OKX REST API: получение пар...", flush=True)
    ...
    # ── 6. OKX WS ─────────────────────────────────────────────────────────────
    print("[6/10] OKX WebSocket: валидация (60 сек)...", flush=True)
    t3 = time.time()
    ...
    # ── 7. Gate.io REST ───────────────────────────────────────────────────────
    print("[7/10] Gate.io REST API: получение пар...", flush=True)
    gate_spot_norm, gate_fut_norm = gate_fetch()
    r["gate_spot_total"] = len(gate_spot_norm)
    r["gate_fut_total"]  = len(gate_fut_norm)
    print(f"       Spot: {len(gate_spot_norm)}, Futures: {len(gate_fut_norm)}")

    # ── 8. Gate.io WS ─────────────────────────────────────────────────────────
    print("[8/10] Gate.io WebSocket: валидация (60 сек)...", flush=True)
    t4 = time.time()
    gate_spot_native, gate_fut_native = gate_load_native()
    agate_spot, agate_fut = gate_ws(gate_spot_native, gate_fut_native, gate_spot_norm, gate_fut_norm)
    r["gate_spot_active"] = len(agate_spot)
    r["gate_fut_active"]  = len(agate_fut)
    print(f"       Spot active: {len(agate_spot)}, Futures active: {len(agate_fut)}  ({time.time()-t4:.0f}s)")

    # ── 9. Пересечения ────────────────────────────────────────────────────────
    print("[9/10] Создание пересечений (combination/)...", flush=True)
    active = {
    ...
    # ── 10. Файлы подписки ────────────────────────────────────────────────────
    print("[10/10] Создание файлов подписки (subscribe/)...", flush=True)
